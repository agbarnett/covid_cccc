...
We show all patients with a stroke and a random selection of 40 patients without a stroke as plotting all patients without a stroke would make the plot too busy.
...
```{r, include=FALSE}
# just interested in transitions to stroke
my.data.stroke = filter(my.data, from!='stroke') 
```


### Joint model for time to stroke using APTT

```{r, joint_model_aptt}
joint_ests_aptt = joint_survival(indata.surv=my.data.stroke, indata.long=daily, long_var='aptt', log_long=TRUE, censor_day=censor_day)
# table of estimates
table_nums(name="joint_aptt","Estimates from the joint model of APTT over time and survival.", level=1) # title, increase first number
joint_ests_aptt$table
# for text below
mult = filter(joint_ests_aptt$ests, model=='Survival', term=='aptt') 
```

The estimates use `r joint_ests_aptt$numbers$N_patients` patients with `r format(joint_ests_aptt$numbers$N_long, big.mark=',')` longitudinal observations. The survival model used `r joint_ests_aptt$numbers$N_event` strokes.
...
##### Longitudinal plot of APTT for stroke and non-stroke patients

```{r}
to_plot = joint_ests_aptt$graph +
  ylab('APTT (log scale)')+
...
### Joint model for time to stroke using ph

ph was the only variable that was not log-transformed as the original distribution did not have a strong skew.

```{r, joint_model_ph}
joint_ests_ph = joint_survival(indata.surv=my.data.stroke, indata.long=daily, long_var='p_h', log_long=FALSE, censor_day=censor_day, scale=0.1)
# table of estimates
table_nums(name="joint_ph","Estimates from the joint model of ph over time and survival.")
joint_ests_ph$table
# for text below
mult = filter(joint_ests_ph$ests, model=='Survival', term=='p_h') 
```

The estimates use `r joint_ests_ph$numbers$N_patients` patients with `r format(joint_ests_ph$numbers$N_long, big.mark=',')` longitudinal observations. The survival model used `r joint_ests_ph$numbers$N_event` strokes.
...
##### Longitudinal plot of ph for stroke and non-stroke patients

```{r}
to_plot = joint_ests_ph$graph +
  ylab('ph')+
  xlab('Day since ICU admission')
figure_nums(name="joint_ph","ph over time for stroke and non-stroke patients.")
print(to_plot)
```


### Joint model for time to stroke using PaO2

```{r, joint_model_pa_o2}
# pa_o2 is skewed so log-transform 
joint_ests_pa_o2 = joint_survival(indata.surv=my.data.stroke, indata.long=daily, long_var='pa_o2', log_long=TRUE, censor_day=censor_day)
# table of estimates
table_nums(name="joint_pao2","Estimates from the joint model of PaO2 over time and survival.") # title
joint_ests_pa_o2$table
# for text below
mult = filter(joint_ests_ph$ests, model=='Survival', term=='pa_o2') 
```

The estimates use `r joint_ests_pa_o2$numbers$N_patients` patients with `r format(joint_ests_pa_o2$numbers$N_long, big.mark=',')` longitudinal observations. The survival model used `r joint_ests_pa_o2$numbers$N_event` strokes.
...
##### Longitudinal plot of PaO2 for stroke and non-stroke patients

```{r}
to_plot = joint_ests_pa_o2$graph +
  ylab('PaO2 (log scale)')+
  xlab('Day since ICU admission')
figure_nums(name="joint_pa_o2","PaO2 over time for stroke and non-stroke patients.") 
print(to_plot)
```

### Joint model for time to stroke using PaCO2

```{r, joint_model_pa_co2}
# pa_co2 is skewed so log-transform
joint_ests_pa_co2 = joint_survival(indata.surv=my.data.stroke, indata.long=daily, long_var='pa_co2', log_long=TRUE, censor_day=censor_day)
# table of estimates
table_nums(name="joint_pa_co2","Estimates from the joint model of PaCO2 over time and survival.") # title
joint_ests_pa_co2$table
# for text below
mult = filter(joint_ests_pa_co2$ests, model=='Survival', term=='pa_co2') 
```

The estimates use `r joint_ests_pa_co2$numbers$N_patients` patients with `r format(joint_ests_pa_co2$numbers$N_long, big.mark=',')` longitudinal observations. The survival model used `r joint_ests_pa_co2$numbers$N_event` strokes.
...
##### Longitudinal plot of PaCO2 for stroke and non-stroke patients

```{r}
to_plot = joint_ests_pa_co2$graph +
  ylab('PaCO2 (log scale)')+
  xlab('Day since ICU admission')
figure_nums(name="joint_pa_co2","PaCO2 counts over time for stroke and non-stroke patients.") 
print(to_plot)
```

### Joint model for time to stroke using platelet count

```{r, joint_model_platelet}
# platelet_count is skewed so log-transform 
joint_ests_platelet = joint_survival(indata.surv=my.data.stroke, indata.long=daily, long_var='platelet_count', log_long=TRUE, censor_day=censor_day)
# table of estimates
table_nums(name="joint_platelet","Estimates from the joint model of Platelet counts over time and survival.") # title
joint_ests_platelet$table
# for text below
mult = filter(joint_ests_platelet$ests, model=='Survival', term=='platelet_count') 
```

The estimates use `r joint_ests_platelet$numbers$N_patients` patients with `r format(joint_ests_platelet$numbers$N_long, big.mark=',')` longitudinal observations. The survival model used `r joint_ests_platelet$numbers$N_event` strokes.

A higher platelet count increases the risk of stroke. A doubling of platelet count multiplies the median time to stroke by `r mult$mean` (95% credible interval `r mult$lower` to `r mult$upper`).

##### Longitudinal plot of platelet count for stroke and non-stroke patients

```{r}
to_plot = joint_ests_platelet$graph +
