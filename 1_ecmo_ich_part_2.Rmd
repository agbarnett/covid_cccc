...
```{r}
figure_nums(name="cox_zph_age", "Plot of the estimated effect of age on dischage over time in ICU. The green line is the estimated effect with dotted lines for a 95% confidence interval. The horizontal red line indicates no effect of age.")
...
my.data = select(my.data, -stroke_type) %>%
  filter(sex != 'Not specified')
# add split to data at 20 days
my.data2 <- survSplit(Surv(time=entry, time2=exit, event=to==5) ~ ., data= my.data, cut=c(20), episode= "tgroup", id="idx")
m = coxph(Surv(time=entry, time2=exit, event) ~ sex + age:strata(tgroup) + bmi, data=my.data2)
summary(m)
```
