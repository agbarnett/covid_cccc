...
patients = select(patients, 'pin', 'site_name','country','age', 'sex', 'bmi','ethnicity', 'pregnant',
                  ...
                  'date_death','date_discharge','last_date',
...
This report uses the data from `r data_date$month` `r data_date$day`. This report uses the scrambled dummy data.
...
# c) stroke after ICU discharge
f3 = filter(f2, complication_stroke_date <= date_discharge | !is.na(date_discharge))
n_stroke_post_icu = nrow(f2) - nrow(f3)
n_final = nrow(f3)
```

We start with `r n_start` patients who had a stroke.
Of these patients `r n_missing_stroke_date` have no stroke date, `r n_stroke_prior_icu` patients had their stroke prior to their ICU admission, and `r n_stroke_post_icu` patients had their stroke after their ICU discharge.
This gives `r n_final` patients with a known stroke date during their ICU admission.
...
my.data = time_transitions(
  ...
  select(pin, from, to, start, end, date_icu, age, complication_stroke, stroke_type, stroke_group, sex, bmi, site_name, country) %>%
...
The two panels of the plot are for the states of "ICU admission" and "stroke". The lines show where patients move to from these states.

There is a clear higher risk of death and lower risk of discharge for patients with a stroke.
...
n.chains= 2
MCMC = 2000 # increase to 10,000 for final run
thin = 2 # increase to 5
debug = TRUE
...
The hazard ratio axis is on log scale (base 10). Each row is a site and there are two estimates per site: one for death and one for discharge. The sites are ordered from lowest to highest risk. The plot has been split into two panels because of the number of sites.
...
run_cox_models = function(in_discharge, in_death, digits=2){
  ...
  c_disch = coxph(Surv(days, event=cens==0, type='right') ~ sex + age + bmi + stroke + cluster(site), data=in_discharge)
  ...
  in_death = mutate(in_death,
            days = ifelse(is.na(days)==TRUE, cens, days)) # get days from censored or uncensored
  c_death = coxph(Surv(days, event=cens == 0, type='right') ~ sex + age + bmi + stroke + cluster(site), data=in_death)
