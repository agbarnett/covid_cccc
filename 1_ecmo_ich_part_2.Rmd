...
This gives `r n_final` patients with a known stroke date during their ICU admission (_to check, should this be hospital?_).
...
```{r, comorbid_table}
co_vars = c(  'comorbidity_chronic_kidney_disease','comorbidity_malignant_neoplasm','comorbidity_smoking')
ftab = desc_table_cat(indata = patients, 
                      vars = co_vars, 
                      group_var = 'stroke_group', 
                      var_levels = c('Hemorrhage','Ischemic','Other/Unknown','None'),
...
#
time_vars = c('days_symptoms_hosp','days_hosp','duration_ecmo','duration_mv')
#
...
for_tab = cox_models$table %>% 
  ...
  mutate(term = ifelse(term=='stroke_o', 'Stroke - Other/Unknown', term),
