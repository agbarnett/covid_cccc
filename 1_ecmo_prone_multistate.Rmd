...
#
ftab = desc_table_cont(indata=patients, vars=time_vars, group_var='any_prone', var_levels = c('No','Yes'), heading=c('No prone','Some prone')) # see 99_functions.R
...
cat('model{
    for (i in 1:N){
      days[i] ~ dweib(r, mu[i])I(cens[i],) # right-censoring
      log(mu[i]) <- int + beta[1]*sex[i] + beta[2]*age[i] + beta[3]*calendar_time[i] + beta[4]*bmi[i] + beta[5]*prone[i] + beta[6]*prone_before[i]+ centre[site[i]] + patient[id[i]]
      bmi[i] ~ dnorm(mu.bmi, tau.bmi) # missing BMI
      prone_before[i] ~ dbern(p) # missing prone before
    }
    ## priors
...
# initial values for single variable outcome (use mean for intercept)
inits = list(r=1, beta=rep(0,6), int=0, tau.centre=1, centre=rep(0, for_death$M), patient=rep(0, for_death$P), tau.id=1, mu.bmi=0, tau.bmi=1, p=0.74) 
inits = rep(list(inits), n.chains) # repeat initial values per chain
