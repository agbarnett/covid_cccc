...
run_cox_models = function(in_discharge, in_death, digits=2, cumulative_prone=FALSE){
  ...
  if(cumulative_prone == TRUE){
    c_disch = coxph(Surv(time=in_discharge$entry, time2=in_discharge$exit, event= to == 3, type='right') ~ in_discharge$sex + in_discharge$age + in_discharge$calendar_time + bmi + in_discharge$prone + in_discharge$cum_prone + cluster(in_discharge$site_num))
  }
...
#  m_death = survreg(Surv(days, event=in_death$cens==0, type='right') ~ bmi + in_death$age + in_death$sex + in_death$calendar_time + in_death$prone + cluster(in_death$site_num))
  ...
  if(cumulative_prone == TRUE ){
    c_death = coxph(Surv(time=in_discharge$entry, time2=in_discharge$exit, event=to == 3, type='right') ~ bmi + in_death$age + in_death$sex + in_death$calendar_time + in_death$prone + in_death$cum_prone + cluster(in_death$site_num))
  }
...
# Model of time on prone

Here we fit an alternative multi-state survival model to examine whether the amount of time on prone has an additional impact.


```{r, include=FALSE}
my.data = time_transitions_ecmo_prone(indata=patients, 
                                      censor_day = censor_day, 
                                      prone_time_dependent = TRUE,
                                      check = TRUE) %>% # from 99_functions.R
  select(pin, from, to, start, end, date_ecmo, age, any_prone, sex, bmi, site_name, site_num, country, ecmo_prone_before, prone_cum) %>%
  rename(id = pin, # rename to suit Martin's use
         entry = start,
         exit = end)

# run models (not clock reset)
for_discharge = mutate(my.data, 
                       to = ifelse(to==3, to, 0)) # censor everything but discharge
for_tab = run_cox_models(in_discharge = for_discharge, in_death = for_death, cumulative_prone = TRUE)$table %>% # run above function
  select(term, HR1, CI1, HR2, CI2) 
```
