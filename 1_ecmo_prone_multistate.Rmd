...
for (k in 1:nrow(my.data.cens)){
  ...
  f = data.frame(pin=this_row$id, days=seq(this_row$entry, this_row$exit-1, 1), state=as.numeric(this_row$from))
  f_last = data.frame(pin=this_row$id, days=this_row$exit, state=as.numeric(this_row$to))
...
times_censor = survival::survfit(Surv(time=entry, time2=exit, event= to==0) ~ 1 + cluster(id), data=my.data) # event is censored
