...
## ---- transitions
my.data = time_transitions_ecmo_prone(patients, censor_day = censor_day, check=TRUE) %>% # from 99_functions.R
...
ftab = flextable(tab) %>%
  add_header('ecmo_prone_before'='', 'No' = 'Any prone during ECMO', "Yes" = 'Any prone during ECMO') %>%
  merge_h(i=1, part = "header") %>% # merge "To" column headers
  theme_box() %>%
  autofit()
table_nums(name="prone_table", "Prone before and during ECMO", level=2) # title
ftab
```
...
prepare_survival = function(indata, cens_events, version='winbugs'){
...
We scaled continuous variables so that the hazard ratios were more clinically relevant, e.g., age per five year increase.
...
To verify our parametric survival models we also fitted non-parametric Cox models to check that they gave similar hazard ratios and confidence intervals. We used a leave-one-country out sensitivity analysis to check that the results were not dependent on one country.
...
run_cox_models = function(in_discharge, in_death, digits=2){
  # a) discharge
  days = ifelse(is.na(in_discharge$days)==TRUE, in_discharge$cens, in_discharge$days)
  bmi = ifelse(is.na(in_discharge$bmi)==TRUE, 0, in_discharge$bmi) # simple imputation of mean
  # weibull - not used
  ...
  # cox
  c_disch = coxph(Surv(days, event=in_discharge$cens==0, type='right') ~ in_discharge$sex + in_discharge$age + in_discharge$calendar_time + bmi + in_discharge$prone + cluster(in_discharge$site_num))
  ...
  # b) death
  days = ifelse(is.na(in_death$days)==TRUE, in_death$cens, in_death$days)
  bmi = ifelse(is.na(in_death$bmi)==TRUE, 0, in_death$bmi)
  # weibull - not used
#  m_death = survreg(Surv(days, event=in_death$cens==0, type='right') ~ bmi + in_death$age + in_death$sex + in_death$calendar_time + in_death$prone + cluster(in_death$site_num))
  # cox
  c_death = coxph(Surv(days, event=in_death$cens==0, type='right') ~ bmi + in_death$age + in_death$sex + in_death$calendar_time + in_death$prone + cluster(in_death$site_num))
...
for_tab = run_cox_models(in_discharge = for_discharge, in_death = for_death)$table %>% # run above function
...
The estimates remain similar regardless of which country is left-out, hence the results are not reliant on patients from one country.
