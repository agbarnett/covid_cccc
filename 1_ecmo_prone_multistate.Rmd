...
load('data/COVID_Data_stroke_Oct_31.RData') # database freez
...
patients = filter(patients,
                  ...
                  #pin != 'OX_007100057', # without influential patient
                  age >= 18, # adults
...
patients = mutate(patients, 
                  # any prone during ECMO:
                  any_prone = as.numeric(!is.na(prone_start) & (prone_start>=date_ecmo | prone_end > date_ecmo)), # start date not missing and after ECMO -> 1
                  any_prone = ifelse(prone_end == date_ecmo, 0, any_prone), # assume supine if ends on same day as ECMO start
                  any_prone = ifelse(is.na(prone_start), 0, any_prone), # no if prone start is missing
...
n_sites = length(unique(patients$site_name))
...
## for checking
# check = select(patients, pin, date_icu, date_death, date_hospital_discharge, last_date, date_outcome, outcome) %>% filter(!is.na(date_icu), is.na(date_death), is.na(date_hospital_discharge), is.na(date_outcome))
```
...
## ---- transitions
my.data = time_transitions(indata=patients, 
                           start_state='date_ecmo',
                           int_state = 'prone_start',
                           int_state_end = 'prone_end', 
                           discharge_date = 'date_hospital_discharge',
                           censor_day = censor_day, check=TRUE) %>% # from 99_functions.R
  select(pin, from, to, start, end, date_ecmo, age, any_prone, sex, bmi, site_name, country, ecmo_prone_before) %>%
  rename('entry' = 'start',
         'exit' = 'end')

# alternative function to verify, dummy function, do not run
alternative = function(){
  library(tidyr)
my.data.alt = time_transitions_string(indata=patients,
                intermediate_prior_ongoing= TRUE,
                censor_day = censor_day,
                start_state = 'date_ecmo',
                int_state = 'prone_start',
                int_state_end = 'prone_end',
                discharge_date = 'date_hospital_discharge',
                dates= c('date_ecmo', 'prone_start', 'prone_end', 'date_hospital_discharge', 'date_transfer','date_death'))
## remove some transitions
# no outcomes after transfer
index = my.data.alt$from == 'date_transfer' 
my.data.alt = my.data.alt[!index,]
# 
index = my.data.alt$from == 'date_ecmo' & my.data.alt$to=='date_int_state_start'& my.data.alt$entry==0&my.data.alt$exit==0.5
my.data.alt = my.data.alt[!index,]
#
index = my.data.alt$from == 'date_int_state_start' & my.data.alt$to=='date_int_state_end'& my.data.alt$entry==0&my.data.alt$exit==0.5
my.data.alt = my.data.alt[!index,]
# rename states
my.data.alt = rename(my.data.alt,
                     'from_c' = 'from',
                     'to_c' = 'to') %>%
  mutate(to = case_when(
    to_c == 'censored' ~ 0, # censored
    to_c == 'date_transfer' ~ 0, # censored
    to_c == 'last_date' ~ 0, # censored
    to_c == 'date_int_state_end' ~ 1,
    to_c == 'date_int_state_start' ~ 2,
    to_c == 'discharge_date' ~ 3,
    to_c == 'discharge' ~ 3,
    to_c == 'date_death' ~ 4,
    to_c == 'death' ~ 4
  ),
  from = case_when(
    from_c == 'date_ecmo' & to_c=='date_int_state_end' ~ 2,
    from_c == 'date_ecmo' & to_c!='date_int_state_end' ~ 1,
    from_c == 'date_int_state_start' ~ 2,
    from_c == 'date_int_state_end' ~ 1,
  ))
# compare and check
with(my.data.alt, table(from_c, to_c))
with(my.data.alt, table(from, to))
#with(my.data, table(from, to))
#f1 = filter(my.data.alt, from==2, to==4) %>% select(pin, from_c, to_c, entry, exit)
#f2 = filter(my.data, from==2, to==4) %>% select(id, from, to, entry, exit)
#f1$pin[f1$pin %in% f2$id == FALSE]
#f2$id[f2$id%in%f1$pin  == FALSE]
#f3 = filter(patients, pin=='OX_00549-0048') %>% select(pin, 'date_ecmo', 'prone_start', 'prone_end', 'date_hospital_discharge', 'date_death')
#f4 = filter(my.data.alt, pin=='OX_00549-0048') %>% select(pin, from_c, to_c, from, to, entry, exit)
#f5 = filter(my.data, id=='OX_00549-0048') %>% select(id, from, to, entry, exit)
} # end of if

# Set transition matrix for mstate
tra <- transMat(x = list(c(2,3,4), c(1,3,4), c(),c()),
                names = c("ECMO", "Prone", "Discharge","Death"))
# version with no transitions from prone to discharge
...
my.data$from = as.character(my.data$from) # needed for function below
...
my.data_ext <- ext_mstate(my.data, tra, id.var='pin')
# output for check
...
```{r}
cont_vars = c('compliance_respiratory_system','eotd_peep','pa_co2','pa_o2','eotd_fi_o2','pa_o2_fi_o2')
ftab = desc_table_cont(indata=daily_ecmo, vars=cont_vars, group_var='any_prone', var_levels=c('No','Yes'), heading=c('No prone','Some prone')) %>% # see 99_functions.R
...
ftab = desc_table_cat(indata = patients, 
                      vars = c('sex','ethnicity','country','pre_ecmo_vent','ecmo_prone_before'), 
                      group_var = 'any_prone', 
                      var_levels = c('No','Yes'),
                      include_total = TRUE,
                      heading = c('No prone','Some prone')) # see 99_functions.R
...
patients = mutate(patients,
                  ...
                  prone_start_post_ecmo = ifelse(prone_start < date_ecmo & !is.na(prone_start), date_ecmo, prone_start), # ignore prone days prior to ECMO start
                  duration_prone = as.numeric(prone_end - prone_start_post_ecmo) + 0.5,
                  duration_prone = ifelse(prone_end <= date_ecmo, 0, duration_prone), # set to zero if prone ended on or before ECMO
                  duration_prone = ifelse(is.na(duration_prone), 0, duration_prone), # set missing to zero
...
#
ftab = desc_table_cont(indata=patients, vars=time_vars, group_var='any_prone', var_levels = c('No','Yes'), heading=c('No prone','Some prone')) %>%
  fontsize(size=10, part='all') %>%
  autofit()
...
The duration of prone is only the days after starting ECMO.
...
time_dates = select(patients, pin, date_ecmo, date_ecmo_discontinued, prone_start, prone_end)
denom = numer = NULL
for (k in 1:nrow(time_dates)){
  if(!is.na(time_dates$date_ecmo[k])&!is.na(time_dates$date_ecmo_discontinued[k])){
    if(time_dates$date_ecmo_discontinued[k] >= time_dates$date_ecmo[k]){ # remove few errors
      denom = c(denom, seq(time_dates$date_ecmo[k], time_dates$date_ecmo_discontinued[k], 1))
    }
  }
...
multistate_diagram(states=states, links = M, pos = c(1, 2, 1), arr.pos=0.65, box.size = 0.14) # see 99_functions.R
...
table_header <- data.frame(
  col_keys = c('from', 'cens','1','2','3','4'),
  h1 = c('', rep("State moved to", 5)),
  h2 = c('State moved from', 'Censored','Supine','Prone','Discharge','Death'),
...
### Model diagram (for models of prone)
...
figure_nums(name="model_diagram2", "The three states in the model and the possible movements between states", level=1) # title
multistate_diagram(states=states, links = M, pos = c(1, 2, 1), arr.pos=0.65, box.size = 0.14) # see 99_functions.R
jpeg(filename='figures/prone/state_diagram_prone.jpg', width=4, height=4, units='in', res=400, quality = 100)
multistate_diagram(states=states, links = M, pos = c(1, 2, 1), arr.pos=0.65, box.size = 0.14) # see 99_functions.R
invisible(dev.off())
```
...
for_prone = filter(my.data, 
                   from==1 | (from==2 & entry==0)) %>% # only transitions from not-prone or prone at time zero
...
for_prone = mutate(for_prone,
                   site_num = as.numeric(as.factor(site_name)), # site number for winbugs
...
n.chains= 2
MCMC = 20000 # increase to 10,000 for final run
thin = 5 # 
...
Patients with a higher BMI have a reduced hazard of being put in a prone position (HR = `r beta$mean[4]`).
...
## Survival model for prone positioning without random site effect

Here we repeat the above model but without the random site effect.

```{r, winbugs_model}
# bugs model
bfile = 'bugs_survival_prone_no_site.txt'
bugs = file(bfile, 'w')
cat('model{
    for (i in 1:N){
      days[i] ~ dweib(r, mu[i])I(cens[i],) # right-censoring
      log(mu[i]) <- int + beta[1]*sex[i] + beta[2]*age[i] + beta[3]*calendar_time[i] + beta[4]*bmi[i] + beta[5]*prone_before[i]
      bmi[i] ~ dnorm(mu.bmi, tau.bmi) # missing BMI
      prone_before[i] ~ dbern(p) # missing prone before
    }
    # priors
...
```{r, include=FALSE}
# set up survival data for winbugs (without site)
bdata = list(N = N, 
             days = for_prone$days,
             cens = for_prone$cens,
             prone_before = as.numeric(for_prone$ecmo_prone_before=='Yes'),
...
# initial values for single variable outcome (use mean for intercept)
inits = list(r=1, beta=rep(0,5), p=0.5, int=0, mu.bmi=0, tau.bmi=1) 
inits = rep(list(inits), n.chains) # repeat initial values per chain

## run WinBUGS
parameters = c('r','beta','int','bmi')
# do not run model if there are already estimates (save time)
any_model = length(dir(path='results', pattern='prone_no_site')) > 0
# run model
if(any_model == FALSE){
  bugs_results_prone_no_site = bugs(data=bdata, inits=inits, parameters=parameters, model.file=bfile, DIC=TRUE, n.chains=n.chains, n.iter=MCMC*thin*2, n.thin=thin, bugs.seed=87947, debug=debug, bugs.directory=bugs_location)
  save(bugs_results_prone_no_site, file='results/survival_prone_no_site.RData')
}
if(any_model == TRUE){
  load('results/survival_prone_no_site.RData')
}
# 
```
...
## tabulate the results
# get beta estimates
beta = bugs_extract(inbugs=bugs_results_prone_no_site, var='beta', exp=TRUE, names=c('Male','Age (+10 years)','Calendar time (+30 days)','BMI (+5 kg/m2)',"Prone before ECMO"), digits=2, make.ci=TRUE) # from 99_functions.R
ftab = rename(beta, 'HR' = 'mean') %>%
  flextable() %>%
  theme_box() %>% # nice theme
  align(align='center', part='all') %>% # centre text
  autofit()
table_nums(name="hr_weibull_prone", "Hazard ratios and 95% credible intervals for prone position from the Weibull model with no site effect")
ftab
```
...
### Check Weibull assumption for time to prone

```{r}
# see http://www.drizopoulos.com/courses/emc/ep03_%20survival%20analysis%20in%20r%20companion
# a) refit model using R (non-Bayesian)
for_residual = mutate(for_prone, 
                      ecmo_prone_before = ifelse(is.na(ecmo_prone_before), 2, ecmo_prone_before),
                      bmi = ifelse(is.na(bmi), 30, bmi)) # simple replace of missing with median
#                      )
fit_weib <- survreg(Surv(exit+0.001, event=to =='2') ~ bmi + sex + age + date_ecmo + ecmo_prone_before, data = for_residual) # event is prone
# b) make residuals
fitted_values <- fit_weib$linear.predictors
resids <- (log(fit_weib$y[, 1]) - fitted_values) / fit_weib$scale
# c) plot
resKM <- survfit(Surv(resids, event=to =='2') ~ 1, data = for_residual)
...
my.data_ext$id = as.numeric(as.factor(my.data_ext$pin)) # needed for function
...
LOS_mat_tab  = select(LOS_mat_tab, from, everything()) # from from to start
#
...
ftab = flextable(LOS_mat_tab) %>%
  ...
  align(align='center', part='all') %>% # centre text
  autofit()
table_nums('elos', 'Expected length of stay in days', level=1) # title, increase level
ftab
```
...
plot_it = function(){
  ...
  colours = c("khaki1","indianred1","cornflowerblue","gray")
  ...
  # legend at top
  par(xpd=TRUE)
  labels = c('ECMO & supine','ECMO and prone','Discharge','Death')
  legend(0, 1.2, labels, col=colours, border=NA, fill=colours, bty='n', ncol=2)
}
plot_it()
...
cum_plot
jpeg('figures/prone/competing_risks_ecmo_prone.jpg', width=7, height=4.5, units='in', res=400, quality=100)
print(cum_plot)
invisible(dev.off())
```
...
cat('model{
  for (i in 1:N){
    days[i] ~ dweib(r, mu[i])I(cens[i],) # right-censoring
    log(mu[i]) <- int + beta[1]*sex[i] + beta[2]*age[i] + beta[3]*calendar_time[i] + beta[4]*bmi[i] + beta[5]*prone[i] + beta[6]*prone_before[i]+ centre[site[i]]
    bmi[i] ~ dnorm(mu.bmi, tau.bmi) # missing BMI
    prone_before[i] ~ dbern(p) # missing prone before
  }
  ## priors
...
bugs_data = mutate(indata,
    id_num = as.numeric(as.factor(pin)), # make ID number for patients
    site_num = as.numeric(as.factor(site_name)), # make ID number for sites
    # clock reset
...
bdata = list(N = N, 
             ...
             prone_before = as.numeric(bugs_data$ecmo_prone_before=="Yes"),
...
thin = 5 # increase to 5 for final run

# initial values for single variable outcome (use mean for intercept)
inits = list(r=1, beta=rep(0,6), int=0, tau.centre=1, centre=rep(0, for_death$M), mu.bmi=0, tau.bmi=1, p=0.74) 
inits = rep(list(inits), n.chains) # repeat initial values per chain
parameters = c('r','beta','int','centre','tau.centre','bmi') # ,'median','diff'
...
# b) discharge
any_model = length(dir(path='results', pattern='bugs_discharge\\.RData')) > 0
# run model
if(any_model == FALSE){
...
## tabulate the results
# get beta estimates
beta_death = bugs_extract(inbugs=bugs_results_death, var='beta', exp=TRUE, names=c('Male','Age (+10 years)','Calendar time (+30 days)','BMI (+5 kg/m2)','Prone position','Prone before ECMO'), digits=2, make.ci=TRUE) # from 99_functions.R
...
Prone position somewhat reduces the hazard of death (HR = `r for_table$mean1[5]`). 

Older age increases the hazard of death (HR = `r for_table$mean1[2]`). Older age decreases the hazard of discharge (HR = `r for_table$mean2[2]`), which increases the length of time on ECMO.
...
### Check Weibull assumption for death and discharge
...
# see http://www.drizopoulos.com/courses/emc/ep03_%20survival%20analysis%20in%20r%20companion
# a) refit model using R (non-Bayesian)
for_residual = mutate(my.data, 
                      exit = ifelse(entry > 0, exit - entry, exit), # subtract starting days if delayed start
                      ecmo_prone_before = ifelse(is.na(ecmo_prone_before), 2, ecmo_prone_before), # mode
                      bmi = ifelse(is.na(bmi), 30, bmi)) # simple replace of missing with median
#                      )
fit_weib <- survreg(Surv(exit+0.001, event=to =='4') ~ bmi + sex + age + date_ecmo + I(from==2) + ecmo_prone_before, data = for_residual) # event is death
# b) make residuals
fitted_values <- fit_weib$linear.predictors
resids <- (log(fit_weib$y[, 1]) - fitted_values) / fit_weib$scale
# c) plot
resKM <- survfit(Surv(resids, event=to =='4') ~ 1, data = for_residual)
plot(resKM, mark.time = FALSE, xlab = "AFT Residuals", ylab = "Survival Probability")
xx <- seq(min(resids), max(resids), length.out = 35)
yy <- exp(- exp(xx))
lines(xx, yy, col = "red", lwd = 2)
...
# see http://www.drizopoulos.com/courses/emc/ep03_%20survival%20analysis%20in%20r%20companion
# a) refit model using R (non-Bayesian)
fit_weib <- survreg(Surv(exit+0.001, event=to =='3') ~ bmi + sex + age + date_ecmo + I(from==2) + ecmo_prone_before, data = for_residual) # event is death
# b) make residuals
fitted_values <- fit_weib$linear.predictors
resids <- (log(fit_weib$y[, 1]) - fitted_values) / fit_weib$scale
# c) plot
resKM <- survfit(Surv(resids, event=to =='3') ~ 1, data = for_residual)
plot(resKM, mark.time = FALSE, xlab = "AFT Residuals", ylab = "Survival Probability")
xx <- seq(min(resids), max(resids), length.out = 35)
yy <- exp(- exp(xx))
lines(xx, yy, col = "red", lwd = 2)
...
my.data_crr = mutate(my.data, 
                     ...
                     prone_before = as.numeric(ecmo_prone_before=='Yes'),
...
my.data_crr = mutate(my.data_crr,
                     ...
                     prone_before = ifelse(is.na(prone_before)==TRUE, 1, prone_before)) # use mode
#
cov1 = as.matrix(select(my.data_crr, sex, age, calendar_time, bmi, prone, prone_before))
# death model
...
ftable = mutate(ftable, Variable=c('Male','Age (+10 years)','Calendar time (+30 days)','BMI (+5 kg/m2)',"Prone position","Prone before ECMO")) %>%
...
```{r}
# turn my.data into long data (daily)
long = NULL
my.data.cens = mutate(my.data,
                      to = ifelse(to=='cens', '0', to)) %>%
  filter(exit - entry > 0.5) # remove few small jumps
for (k in 1:nrow(my.data.cens)){
  this_row = my.data.cens[k,]
  f = data.frame(pin=this_row$pin, days=seq(this_row$entry, this_row$exit-1, 1), state=as.numeric(this_row$from))
  f_last = data.frame(pin=this_row$pin, days=this_row$exit, state=as.numeric(this_row$to))
  long = bind_rows(long, f, f_last)
}
# quick check
check = group_by(long, days, state) %>%
  tally()
cplot = ggplot(data=check, aes(x=days, y=n, fill=factor(state)))+
  geom_bar(stat='identity')
#cplot

# get daily data
small = select(patients, pin, date_ecmo)
to_plot = full_join(daily, small, by='pin') %>%
  mutate(days = as.numeric(date_daily - date_ecmo)) %>%
  filter(!is.na(days),
         days >= 0,
         days <= censor_day) %>%
  select(pin, days, date_daily, glasgow_coma_score, avpu)

# merge with state information
to_plot = left_join(to_plot, long, by=c('pin','days')) %>%
  filter(state %in% c(1,2)) # just supine and prone

# daily mean by state
to_plot_mean = group_by(to_plot, days, state) %>%
  summarise(#meana = mean(avpu, na.rm=TRUE), # categorical
            meang = mean(glasgow_coma_score, na.rm=TRUE)) %>%
  filter(!is.na(meang))
# plot
gplot = ggplot(data=to_plot_mean, aes(x=days, y=meang, col=factor(state)))+
  geom_point()+
  scale_color_manual(NULL, labels=c('Supine','Prone'), values=1:2)+
  geom_smooth(span=0.9)+
  xlab('Days since ECMO start')+
  ylab('Average Glasgow coma score')+
  theme_bw()+
  theme(legend.position = c(0.15,0.85))
figure_nums(name="glasgow_plot", "Plot of daily average Glasgow coma scores by position.") # title
gplot
```


The plot shows a steady rise in score in the supine position. The patterns in the prone group are divergent, but are reliant on a small number of patients.
...
for (i in 1:nrow(my.data)){
  if(nrow(my.data[i,]) > 0){
    f = with(my.data[i,], data.frame(id=pin, state=from, days=entry:(exit-1)))
    flast = with(my.data[i,], data.frame(id=pin, state=to, days=exit))
  }
  if(nrow(my.data[i,]) == 0){
    f = NULL
    flast = with(my.data[i,], data.frame(id=pin, state=to, days=exit))
...
# get site estimates
load('results/survival_prone.RData') # get estimates 
centres = bugs_extract(inbugs=bugs_results_prone, var='^centre', exp=TRUE, arrange=TRUE) %>% # from 99_functions.R
  mutate(over = as.numeric(lower>1) + 1) # for colouring estimates
# annotation
...
cplot
jpeg('figures/prone/left_out_prone.jpg', width=4.5, height=5.5, units='in', res=500, quality=100)
print(cplot)
invisible(dev.off())
```
...
##### Site effects (prone positioning): details on the influential sites

```{r}
# get centre numbers that look high
high = filter(centres, over==2) %>%
  mutate(
    text = row.names(.),
    num = as.numeric(str_remove_all(text, '[^0-9]'))) %>% # get centre number
  pull(num)
# add back to original data
for_prone_original = mutate(for_prone,
                   high = site_num %in% high)
# now get statistics per site
stats = group_by(for_prone_original, site_num, high) %>%
  summarise(n = n(),
            exit = mean(exit, na.rm=TRUE), # average time
            censored = sum(as.numeric(to=='cens')), # number censored
            prone = sum(as.numeric(to=='2'))) # movements to prone
ftab = filter(stats, high==TRUE) %>%
  select(-high) %>%
  flextable() %>%
  theme_box() %>%
  autofit()
...
The table above shows the summary statistics for the three sites that were relative outliers in terms of prone positioning.
...
These models account for correlated results from the same site. We use a simple mean imputation for patients missing BMI.
...
run_cox_models = function(in_discharge, in_death, digits=2, cumulative_prone=FALSE){
  ...
  in_discharge = mutate(in_discharge,
                        ...
                        prone_before = ifelse(is.na(prone_before)==TRUE, 1, prone_before)) # simple imputation of mode
  # weibull - not used
  ...
  if(cumulative_prone == FALSE ){ # censored = 0 (FALSE) is the event
    ...
    c_disch = coxph(Surv(days, event=cens==0, type='right') ~ sex + age + calendar_time + bmi + prone + prone_before + cluster(site), data=in_discharge)
  }
  ...
  if(cumulative_prone == FALSE ){ # censored = 0 (FALSE) is the event
    in_death = mutate(in_death,
                      days = ifelse(is.na(days)==TRUE, cens, days)) # get days from censored or uncensored
    c_death = coxph(Surv(days, event=cens == 0, type='right') ~ bmi + age + sex + calendar_time + prone + prone_before + cluster(site), data=in_death)
  }
  if(cumulative_prone == TRUE ){
...
```{r}
# run simple model of censoring times, recommended by https://arxiv.org/pdf/2005.13271.pdf
times_censor = survival::survfit(Surv(time=entry, time2=exit, event= to==0) ~ 1 + cluster(pin), data=my.data) # event is censored
# report median censoring time
g = ggsurvplot(times_censor, data = my.data,
               legend.labs=NULL,
               xlim=c(0,90),
               break.time.by=30,
                  conf.int =FALSE,
                  censor =FALSE) 
g
figure_nums(name="censor_times", "Kaplan-Meier plot of the censoring times.")
#
cens_stats = surv_median(times_censor)
```

The Kaplan-Meier plot above shows the censoring times. Patients could be censored prior to 90 days based on their last date of observation. Patients were censored at 90 days if they were still in hospital. The median day of censoring is `r cens_stats$median`.
...
index = filter(influence_discharge$data, covariate=='prone', abs(res) >= 0.6) %>% pull(xval)
to_table = cox_models$discharge_data[index,] %>%
  select(site, sex, age, bmi, prone, days, cens) %>%
  mutate(
...
# test of influential observation(s)
...
There are some influential patients for prone for the model examining deaths, although the size is relatively small at around 0.1, meaning the estimated reduced risk of death would be larger without these patients. For comparison the estimate for prone with all the data is `r round(cox_models$death_model$coefficients[5]*100)/100`.
...
The only variable that may be worth investigating is the effect of age on discharge.
...
for (this_country in countries){
  ...
  ests = run_cox_models(in_discharge = for_discharge, in_death = for_death, digits=10)$table %>% # run above function to run Cox models
    filter(str_detect(term, 'prone$')) %>% # not prone before
...
The estimates change when the USA is left out. This is because the one very influential patient is from the USA.
